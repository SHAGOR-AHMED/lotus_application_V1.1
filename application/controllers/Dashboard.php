...
class Dashboard extends CI_Controller {
	...
	public function index() {
		...
		$data['title'] = "LOTUS Application | Dashboard";
	...
	public function guardian_dashboard() {

		$data['title'] = "OHMS | Guardian Dashboard";
		$data['notices'] = $this->dashboard_model->get_current_notice();
		$MemberID = $this->session->userdata('member_id');
		$data['guardian_info'] = $this->db->select('*')->from('tbl_members')->where('mem_id',$MemberID)->get()->row();
		$this->load->view('backend/template_header', $data);
		$this->load->view('backend/template_left');
		$this->load->view('backend/guardian_dashboard');
		$this->load->view('backend/template_footer');
	}
